function [parameter_iteration, out] = PATPEMS(Np, S, bound, logpdf_handle, options)
%PATPEMS   SMC with (i) a robust rCESS-band adaptive sequence
...
% -------------------------------------------------------------------------
% Syntax
%   parameter_iteration = PATPEMS(Np, S, bound)
%   [parameter_iteration, out] = PATPEMS(Np, S, bound)
%   [parameter_iteration, out] = PATPEMS(Np, S, bound, @logpdf, options)
%
% Inputs
...
%   options.Beta: adaptive sequence of intermediate distributions
...
%
% Outputs
%   parameter_iteration : [Np x d x K] particle states at thinned stages
%                         (includes the initial beta = 0 particle population).
%
%   out                 : diagnostics struct, thinned to match
%                         parameter_iteration along the third dimension.
%       .beta        : [1 x K] schedule values at stored stages (beta_1 = 0).
%       .ESS         : [1 x K] ESS after reweighting, before resampling.
%       .CESS        : [1 x K] rCESS for the selected dBeta at each stored stage.
%       .dBeta       : [1 x K] beta increments used at stored stages.
%       .resampled   : [1 x K] logical flags indicating whether resampling occurred.
%       .acc_arm     : [1 x K] acceptance rate of the ARM move.
%       .acc_xover   : [1 x K] acceptance rate of the crossover move
%                              measured per individual.
%       .acc_de      : [1 x K] acceptance rate of the DE-MH move.
%       .logZ_cum    : [1 x K] cumulative log-evidence estimate.
%       .calls       : [1 x K] cumulative number of logpdf/model evaluations
%                              up to each stored stage. This counter is tracked
%                              internally by PATPEMS and does not require any
%                              counter in the user-supplied logpdf function.
%       .stage_calls : [1 x K] number of logpdf/model evaluations used within
%                              each stored stage.
%       .S_K         : scalar   number of thinned stages actually output (K).
%       .S_full      : scalar   total number of true stages executed.
%       .timing      : struct   timing information.
...
store_full_idx = nan(1, Kmax);
store_full_idx(k_store) = 1;   % beta = 0 frame
...
calls_cum      = 0;              % cumulative number of logpdf/model evaluations
calls_vec      = zeros(1, S_max);% cumulative calls at each full stage
stage_calls_v  = zeros(1, S_max);% calls used within each full stage
...
while true
    ...
    % Count model/logpdf evaluations in the current stage.
    % This is counted by PATPEMS itself and does not require any counter
    % inside the user-supplied target function.
    calls_stage = 0;
    ...
    % One logpdf evaluation for each current particle
    calls_stage = calls_stage + Np;
...
%         beta_new_raw = x;
        ...
        % --- Power-function schedule (front-fast, back-slow) ---
        gamma = 3.0;
        beta_new_raw = 1 - (1 - x)^gamma;
    
        % --- Exponential schedule (front-slow, back-fast) ---
%         a = 5.0;
%         beta_new_raw = (exp(a * x) - 1) / (exp(a) - 1);
    
        beta_new_raw = min(1.0, max(beta_new_raw, beta_raw + 1e-12));
        dBeta_raw    = max(1e-12, beta_new_raw - beta_raw);
    
        % fabricate stepinfo for printing consistency
    ...
    switch pol
        ...
        case 'ess'
            ...
            band_now = sort(opts.Beta.TargetBand(:)).';
            if isfield(opts.Beta,'BandDecay') && isfield(opts.Beta.BandDecay,'Enabled') && opts.Beta.BandDecay.Enabled
                bt = 0.95;
                if isfield(opts.Beta.BandDecay,'BetaThresh') && ~isempty(opts.Beta.BandDecay.BetaThresh)
                    bt = double(opts.Beta.BandDecay.BetaThresh);
                end
                floorBand = sort(opts.Beta.BandDecay.Floor(:)).';
                if beta_raw > bt
    ...
    if (s+1) <= max(0, round(opts.Beta.WarmupNoResample))
        doResample = false;
    end
        ...
        switch mv
            case "ARM"
                ...
                if opts.Parallel.Enabled
                    ...
                    ll_newarr = ll;              % cached
                    parfor i = 1:Np
                        if strcmpi(opts.Moves.ARM.Boundary, 'reflect')
                            prop = randw(theta(i,:), LB, UB, opts.Moves.ARM);
                        else
                            prop = randw_redraw(theta(i,:), LB, UB, opts.Moves.ARM);
                        end
                ...
                % ARM evaluates one proposal per particle
                calls_stage = calls_stage + Np;
            ...
            case "XOVER"
                ...
                P = size(pair_idx,1);
                acc2 = false(P,1);
                % Crossover evaluates two children only for pairs where crossover occurs
                calls_stage = calls_stage + 2 * sum(will_xo);
                if P > 0
                    if opts.Parallel.Enabled
            ...
            case "DEMH"
                ...
                if opts.Parallel.Enabled
                    parfor k = 1:Np
                        if strcmpi(opts.Moves.DEMH.Boundary, 'reflect')
                            propDE = Generatep_reflect(old_theta, k, LB, UB, opts.Moves.DEMH);
                        else
                            propDE = Generatep_redraw(old_theta, k, LB, UB, opts.Moves.DEMH);
                        end
                ...
                theta = theta_new;
                ll    = ll_newarr;
                % DE-MH evaluates one proposal per particle
                calls_stage = calls_stage + Np;
    ...
    % Update cumulative model-call counter after all moves in this stage
    calls_cum = calls_cum + calls_stage;
    calls_vec(s+1) = calls_cum;
    stage_calls_v(s+1) = calls_stage;
    ...
    if store_now
        if k_store >= size(parameter_iteration,3)
            ...
            store_full_idx = [store_full_idx, nan(1,10)]; %#ok<AGROW>
        end
        k_store = k_store + 1;
        parameter_iteration(:,:,k_store) = theta;
        store_full_idx(k_store) = s + 1;
    end
...
if hit_budget && (beta_last < 1 - 1e-12)
    error('PATPEMS:MaxStagesTooSmall', ...
        ['[PATPEMS] Stage budget (MaxStages = %d) exhausted before reaching beta = 1. ', ...
...
keep_idx = store_full_idx(1:k_store);
...
out.calls       = calls_vec(keep_idx);
out.stage_calls = stage_calls_v(keep_idx);
...
% Beta (adaptive knobs; fixed schedule has no exposed knobs)
opts.Beta = struct();
opts.Beta.Adaptive         = true;          % false => fixed exponential with scalar=1e-6
% Adaptive path
opts.Beta.TargetBand       = [0.70, 0.85];
opts.Beta.MinDeltaBeta     = 1e-3;
opts.Beta.MaxDeltaBeta     = 0.1;
opts.Beta.GrowthFactor     = 2.0;
opts.Beta.WarmupNoResample = 5;
...
opts.Moves.ARM.Boundary  = 'reflect';
opts.Moves.DEMH.Boundary = 'reflect';
...
% Sanity clamps
opts.Beta.MinDeltaBeta = max(1e-6, double(opts.Beta.MinDeltaBeta));
opts.Beta.MaxDeltaBeta = max(opts.Beta.MinDeltaBeta, double(opts.Beta.MaxDeltaBeta));
opts.Thinning               = max(1, round(opts.Thinning));
end

function opts = merge_options(opts, user)
%MERGE_OPTIONS Recursively merge user options into defaults.
...
for i = 1:numel(fn)
    ...
    if isstruct(user.(f)) && isfield(opts, f) && isstruct(opts.(f))
        ...
        opts.(f) = merge_options(opts.(f), user.(f));
    else
        % overwrite scalar / char / cell / non-struct fields
        opts.(f) = user.(f);
    end
end
end
...
function print_pem_smc_config(opts, Np, d, S_in, logpdf_handle)
    ...
    fprintf('\n========== [PATPEMS] Configuration ==========\n');
    fprintf(' Problem size   : Np=%d | d=%d | S(budget)=%d | Thinning=%d\n', ...
            Np, d, S_in, max(1, round(opts.Thinning)));

    if opts.Beta.Adaptive
        fprintf(' Beta      : Adaptive=TRUE | TargetBand=[%.3f, %.3f] | MinDeltaBeta=%.3g | MaxDeltaBeta=%.3g | GrowthFactor=%.3g | WarmupNoResample=%d\n', ...
            opts.Beta.TargetBand(1), opts.Beta.TargetBand(2), ...
            opts.Beta.MinDeltaBeta, opts.Beta.MaxDeltaBeta, opts.Beta.GrowthFactor, ...
            max(0, round(opts.Beta.WarmupNoResample)));
        if isfield(opts.Beta,'BandDecay') && isfield(opts.Beta.BandDecay,'Enabled') && opts.Beta.BandDecay.Enabled
            fprintf('                : BandDecay(beta-threshold) -> BetaThresh=%.3f, Floor=[%.3f, %.3f]\n', ...
                opts.Beta.BandDecay.BetaThresh, ...
                opts.Beta.BandDecay.Floor(1), opts.Beta.BandDecay.Floor(2));
        end

    else
        fprintf(' Beta      : Adaptive=FALSE | Fixed=exponential(scalar=1e-6)\n');
    end
