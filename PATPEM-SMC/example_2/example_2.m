...
S     = 500;               % stage budget (upper bound; loop stops earlier if beta hits 1)
d     = 100;                % dimensionality
bound = [ -10*ones(1,d) ;   % lower bounds
           10*ones(1,d) ];  % upper bounds

% Target log-density handle (1-arg or 2-arg both supported by PATPEMS)
logpdf = @(th) target(th);
...
% 1) Beta (Adaptive rCESS-band schedule)
opts.Beta.Adaptive         = true;          % true => adaptive; false => fixed exponential (scalar=1e-6)
opts.Beta.TargetBand       = [0.8, 0.9];    % per-stage rCESS target band
opts.Beta.MinDeltaBeta     = 1e-6;          % absolute lower bound on step size
opts.Beta.MaxDeltaBeta     = 0.1;           % absolute upper bound on step size
opts.Beta.GrowthFactor     = 1.1;          % cap: dBeta_next <= 1.25 * dBeta_prev
opts.Beta.WarmupNoResample = 2;             % forbid resampling for the first K stages
...
opts.Resample.PeriodK=15;

% 3) Move operators (any subset/order: 'ARM' | 'XOVER' | 'DEMH')
opts.Moves.Sequence = {'XOVER','DEMH','DEMH'};
...
[figs, metrics_plot] = plot_pem_smc_all(parameter_iteration, out, bound, opts, ...
    ...
    'PosteriorTruth', PT);
% plot_marginal_pdf_band_100d(theta_final, 'XLim',[-10 10], 'BandPct',[2.5 97.5], 'Nx',600);
% saveAllFigures('G:\CoLM参数优化\File\WRR(2025.4.29)\2025.9.16\代码整理\代码上传\PATPEMS\example_2','Format','tif');
% save 'G:\CoLM参数优化\File\WRR(2025.4.29)\2025.9.16\图片和程序\example_2\M4\theta_final.mat' theta_final
